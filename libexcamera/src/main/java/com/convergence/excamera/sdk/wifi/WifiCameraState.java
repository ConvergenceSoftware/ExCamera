...
public enum WifiCameraState {
    ...
    //Prepared, it is the transition state, load frame stream success and start looping to load bitmap frame from stream
    Prepared,

    //预览状态，正常预览并且未因连续多帧图片获取失败触发重试状态
    //Previewing，preview is started and frame is available, Retrying state have not been triggered because of failing to acquire consecutive frames
    Previewing
}
